#Developed by github.com/DaniEnsi
...
{self.console_msg("4")} Share Bot {Fore.RED}{self.status["shares"]}
    ...
    def send_bot(self, video_url, bot, div):
        try:
            self.driver.find_element("xpath", self.xpaths[bot]).click()
            time.sleep(0.5)
        except:
            pass
        link = self.driver.find_element("xpath", f"/html/body/div[4]/div[{div}]/div/form/div/input")
        link.clear()
        link.send_keys(video_url)
        self.driver.find_element("xpath", f"/html/body/div[4]/div[{div}]/div/form/div/div/button").click() #Search button
        time.sleep(0.8)
        try:
            self.driver.find_element("xpath", f"/html/body/div[4]/div[{div}]/div/div/div[1]/div/form/button").click() 
        ...
        time.sleep(3)
        try:
            s = self.driver.find_element("xpath", f"/html/body/div[4]/div[{div}]/div/div/span")
    ...
    def main(self):
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW("TokView V2 | Developed by @DaniEnsi on Github")
